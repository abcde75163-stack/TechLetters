...
GOOGLE_SHEET_NAME = get_secret("GOOGLE_SHEET_NAME", "click_logs")
...
def build_click_tracking_url(target_url, campaign_id, link_type, tech_id="", category=""):
    ...
    if not TRACKING_BASE_URL or not GOOGLE_SHEET_ID:
...
def get_click_sheet():
    ...
    info = GOOGLE_SERVICE_ACCOUNT_JSON
    ...
    try:
        ...
        worksheet = spreadsheet.add_worksheet(title=GOOGLE_SHEET_NAME, rows=1000, cols=12)
        worksheet.append_row([
            "clicked_at", "campaign", "link_type", "tech_id", "category",
            "target_url", "user_agent", "source_app"
        ])
    return worksheet


def log_click_to_sheet(campaign, link_type, tech_id, category, target_url):
    worksheet = get_click_sheet()
...
def handle_tracking_request():
    ...
    try:
        log_click_to_sheet(campaign, link_type, tech_id, category, target_url)
